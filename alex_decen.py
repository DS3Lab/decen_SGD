...
def main(_):
  ...
  # Create the model
        ...
        x_ = tf.image.resize_image(x,[224,224], method=tf.image.ResizeMethod.BICUBIC)
        x_reshaped = tf.reshape(x_, [-1, 224, 224, 3])  
        ...
        avg_w1 = tf.assign(W_conv1[FLAGS.task_index], (W_conv1[0]+W_conv1[1]+W_conv1[2]+W_conv1[3])/4.0)
        avg_b1 = tf.assign(b_conv1[FLAGS.task_index], (b_conv1[0]+b_conv1[1]+b_conv1[2]+b_conv1[3])/4.0)
        avg_w2 = tf.assign(W_conv2[FLAGS.task_index], (W_conv2[0]+W_conv2[1]+W_conv2[2]+W_conv2[3])/4.0)
        avg_b3 = tf.assign(b_conv2[FLAGS.task_index], (b_conv2[0]+b_conv2[1]+b_conv2[2]+b_conv2[3])/4.0)
        avg_w3 = tf.assign(W_conv3[FLAGS.task_index], (W_conv3[0]+W_conv3[1]+W_conv3[2]+W_conv3[3])/4.0)
        avg_b3 = tf.assign(b_conv3[FLAGS.task_index], (b_conv3[0]+b_conv3[1]+b_conv3[2]+b_conv3[3])/4.0)
        avg_w4 = tf.assign(W_conv4[FLAGS.task_index], (W_conv4[0]+W_conv4[1]+W_conv4[2]+W_conv4[3])/4.0)
        avg_b4 = tf.assign(b_conv4[FLAGS.task_index], (b_conv4[0]+b_conv4[1]+b_conv4[2]+b_conv4[3])/4.0)
        avg_w5 = tf.assign(W_conv5[FLAGS.task_index], (W_conv5[0]+W_conv5[1]+W_conv5[2]+W_conv5[3])/4.0)
        avg_b5 = tf.assign(b_conv5[FLAGS.task_index], (b_conv5[0]+b_conv5[1]+b_conv5[2]+b_conv5[3])/4.0)
        avg_w6 = tf.assign(W_fc1[FLAGS.task_index], (W_fc1[0]+W_fc1[1]+W_fc1[2]+W_fc1[3])/4.0)
        avg_b6 = tf.assign(b_fc1[FLAGS.task_index], (b_fc1[0]+b_fc1[1]+b_fc1[2]+b_fc1[3])/4.0)
        avg_w7 = tf.assign(W_fc2[FLAGS.task_index], (W_fc2[0]+W_fc2[1]+W_fc2[2]+W_fc2[3])/4.0)
        avg_b7 = tf.assign(b_fc2[FLAGS.task_index], (b_fc2[0]+b_fc2[1]+b_fc2[2]+b_fc2[3])/4.0)
        avg_w8 = tf.assign(W_fc3[FLAGS.task_index], (W_fc3[0]+W_fc3[1]+W_fc3[2]+W_fc3[3])/4.0)
        avg_b8 = tf.assign(b_fc3[FLAGS.task_index], (b_fc3[0]+b_fc3[1]+b_fc3[2]+b_fc3[3])/4.0)
        ...
        sv = tf.train.Supervisor(is_chief=(FLAGS.task_index == 0),
                             logdir="/mnt/ds3lab/litian/logs_3",
                             init_op=init_op)
        ...
        with sv.managed_session(server.target) as sess:
            ...
            for i in range(50000):
                ...
                image_batch = np.reshape(image_batch,[-1,32,32,3])
