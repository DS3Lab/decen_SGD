...
#       print h_conv1.get_shape()
 ...
 #      print max_pool1.get_shape()   
       ...
       # Third fully-connected laye     
        ...
        zipped_data = zip(data_sets['images_train'], data_sets['labels_train'])
        batches = data_helpers.gen_batch(list(zipped_data), 64, 50000)
        #b = tf.placeholder(tf.float32, shape=[None, 32, 32, 3])
        #get_batch = tf.image.resize_images(b,[224,224])
        ...
        with sv.managed_session(server.target) as sess:
            ...
            for i in range(50000):
                ...
                #  new_batch.append(imresize(im,[224,224]
                #image_batch = tf.image.resize_images(image_batch,[224,224]) #return a tensor
                #image_batch = sess.run(image_batch)
                #image_batch = sess.run(get_batch, feed_dict={b:image_batch})
